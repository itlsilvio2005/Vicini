...
const TABS: { id: TabId; label: string; icon: React.ReactNode; short: string }[] = [
  { id: "bacheca", label: "Bacheca Manifesti", short: "Bacheca", icon: <ScrollText size={16} /> },
  { id: "imprese", label: "Le imprese del luogo", short: "Imprese", icon: <Landmark size={16} /> },
  { id: "mappa", label: "Mappa & Luoghi del Territorio", short: "Mappa & Luoghi", icon: <MapIcon size={16} /> },
  { id: "b2c", label: "Le Mie Volontà & Il Nucleo", short: "Volontà & Nucleo", icon: <HeartHandshake size={16} /> },
  { id: "agenzia", label: "Area Riservata Agenzia", short: "Area Agenzia", icon: <Lock size={16} /> },
];

function Shell() {
  const [tab, setTab] = useState<TabId>("bacheca");
  ...
  return (
    <div className="paper-ambient flex min-h-screen flex-col">
      ...
      <header className="sticky top-0 z-50 border-b border-night-700 bg-night-900/97 shadow-lg shadow-night-950/30 backdrop-blur">
        ...
        {/* Tab bar */}
        <nav className="nice-scroll mx-auto mt-3 flex max-w-7xl gap-1 overflow-x-auto px-4 sm:px-6" aria-label="Sezioni della piattaforma">
          {TABS.map((t) => {
            ...
            return (
              <button
                ...
                className={`relative flex shrink-0 items-center gap-2 px-3.5 pb-3 pt-1.5 text-[12.5px] font-semibold tracking-wide transition-colors sm:text-[13px] ${
